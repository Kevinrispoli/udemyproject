...
export default function App(){
  ...
  function handleAddFriend(friend){
    setFriends((friends) => ([...friends, friend]));//create new array with friends
    setShowAddFriend(false);  //closes form after friend is added       
                  }
 ...
 {showAddFriend && <FormAddFriend  onAddFriend={handleAddFriend}/>}
  ...
  </div>                                                //add friend name to forms--for bill---put in formSplitBIll to get it 
 ) ;}//set to null
  ...
  const isSelected = selectedFriend?.id === friend.id;
    return ( <li className={isSelected ? "selected" : ""}> <img src={friend.image} alt={friend.alt} /> 
         <h3>{friend.name}</h3>     
 ...
 return ( <form className="form-add=friend" onSubmit={handleSubmit} >
  ...
  function FormSplitBill({selectedFriend, onSplitBill}){  
    ...
    const paidByFriend = bill ? bill - paidByUser : "";              //recieve selected friend
